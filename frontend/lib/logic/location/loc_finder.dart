...
Future<Position?> getLocationTemp() async {
  ...
  if (perms == LocationPermission.denied) {
    ...
    if (accuracy == LocationAccuracyStatus.reduced) {
      Geolocator.requestTemporaryFullAccuracy(purposeKey: 'We require your precise location to be able to navigate you');
      accuracy = await Geolocator.getLocationAccuracy();
      if (accuracy == LocationAccuracyStatus.reduced) {
  ...
  if (perms == LocationPermission.deniedForever) {
    return Future.error('Location services have been denied forever on this device.');
  }
